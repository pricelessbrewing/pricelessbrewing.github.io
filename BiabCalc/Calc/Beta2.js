function ChangeInVolSparge() {
			MashThickness = (WaterTot - VolSparge) * 4 / GBill;
			byId("MashThickness").style.backgroundColor = "#ffad99";
			byId("VolSparge").style.backgroundColor = "#99ddff";
			control1 = "VolSparge";
			PreviousThickness = MashThickness;
}
	
	
function ChangeInMashThickness() {
	VolSparge = WaterTot - MashThickness * GBill / 4;
			byId("MashThickness").style.backgroundColor = "#99ddff";
			byId("VolSparge").style.backgroundColor = "#ffad99";
			PreviousSparge = VolSparge.toFixed(2);
			control1 = "mashthickness";
...
/*function MashThicknessAndSparge (event){
  if (event.target === MashThickness) {
    ChangeInMashThickness();
  } else {
    ChangeInSpargeVolume();
  }
}

var VolSparge = document.getElementById('VolSparge');
var MashThickness = document.getElementById('MashThickness');
...
var BatchVol,
	...
	MeasuredPreboilTemp,
	MeasuredPreboilGrav,
	MeasuredPreboilTempVol,
	MeasuredChilledVol,
	MeasuredChilledTemp,
	MeasuredChilledGrav,
	TempAdjustmentFirst,
	TempAdjustmentSecond,
	TempAdjustmentBoil,
	TempAdjustmentChilled,
	MeasuredConversion,
	MeasuredLauter,
	MeasuredMash,
	MeasuredBrewhouse,
	MeasuredConversionFeedback,
	MeasuredLauterFeedback,
	MeasuredMashFeedback,
	MeasuredBrewhouseFeedback,
	TotalPoints,
	MashSG,
	MashWortVolume,
	MashWortWeight,
	TempAdjustmentStrike,
	EstLauterMashSG,
	EstLauterMashGrav,
	EstLauterFirstRunningsVolume,
	EstLauterFirstRunEff,
	EstLauterSecRunEff,
	EstLauterTotalEff,
	LauterGBill,
	LauterArray,
	...
	GBillArrayIndex,
	MeasuredSparge,
	EstLauterBrewhouseEff,
	EstLauterMashEff,
	PreviousWaterTot,
	EfficiencyFeedback,
	MeasuredSpargeFeedback;
...
function updateInputs() {
	...
	//This section updates all input variables in case any input variable is changed via JS.
...
//UpdateDisplay is used to convert unit labels, and update outputs
...
function GetEfficiencies(LauterGBill){
	//LauterGBill = GBill;
	//LauterWaterTot = WaterTot;
	//LauterVolSparge = VolSparge;
	//LauterVolStart = VolStart;
	...
	EstLauterMashSG =
			...
			100 *
				MAGPot *
				LauterGBill /
				(MAGPot * LauterGBill + SGSuccrose * 8.3304 * LauterVolStart) /
				(258.6 -
					227.1 * (100 * MAGPot * LauterGBill / (MAGPot * LauterGBill + SGSuccrose * 8.3304 * LauterVolStart) / 258.2));

		EstLauterMashGrav = 10 * (EstLauterMashSG - 1) * MAGEstConv;

		EstLauterFirstRunningsVolume = LauterVolStart - LauterGBill * Gabs - LossTunTrub;

		EstLauterFirstRunEff =  EstLauterFirstRunningsVolume *
			EstLauterMashSG *
			8.3304 /
			(MAGPot / SGSuccrose * LauterGBill + 8.3304 * LauterVolStart);

		EstLauterSecRunEff =
			LauterVolSparge /
			(LauterVolSparge +
				(1 - EstLauterFirstRunEff) *
					(MAGPot * LauterGBill / SGSuccrose + 8.3304 * LauterVolStart) /
					(8.3304 * EstLauterMashSG)) *
			((1 - EstLauterFirstRunEff) * (MAGPot / SGSuccrose) * LauterGBill) /
			((1 - EstLauterFirstRunEff) * (MAGPot / SGSuccrose) * LauterGBill) *
			(1 - EstLauterFirstRunEff);

		EstLauterTotalEff = 100*(EstLauterFirstRunEff + EstLauterSecRunEff);
		EstLauterMashEff = EstLauterTotalEff * MAGEstConv/100;
		EstLauterBrewhouseEff = EstLauterMashEff * (VolChilled / (VolPost / 1.043841336));
...
//Save Gbill, and each efficiency component to array
		...
		LauterVolSparge = 0;
		LauterVolStart = LauterWaterTot;

GetEfficiencies(LauterGBill);

		LauterNoSpargeArray.push(EstLauterTotalEff);
		
		MashNoSpargeArray.push(EstLauterMashEff);
		BrewhouseNoSpargeArray.push(EstLauterBrewhouseEff);
		EstLauterMashEff = EstLauterFirstRunEff * MAGEstConv / 100;
...
function MashAnalysis() {
	...
	TempMashout =
		(TempMash * (GBill + 5 * (GBill * Gabs)) + 5 * TempSparge * VolSparge) / (GBill + 5 * (VolSparge + GBill * Gabs));

	//Measured gravities and efficiencies
	MeasMashPlato =
		-616.868 +
		1111.14 * MeasMashGrav -
		630.272 * MeasMashGrav * MeasMashGrav +
		135.997 * MeasMashGrav * MeasMashGrav * MeasMashGrav;
	MeasGabs = Gabs;
	MeasMashWT = -((VolStart * 8.335 + GBill * MAGMoist) * MeasMashPlato) / (-100 + MeasMashPlato);
	MeasConv = Math.max(100 * MeasMashWT / SugarTot, 0);
	MeasPrebPlato =
		-616.868 +
		1111.14 * MeasPrebGrav -
		630.272 * MeasPrebGrav * MeasPrebGrav +
		135.997 * MeasPrebGrav * MeasPrebGrav * MeasPrebGrav;
	...
	MeasMashPlato2 =
		-616.868 +
		1111.14 * MeasMashGrav2 -
		630.272 * MeasMashGrav2 * MeasMashGrav2 +
		135.997 * MeasMashGrav2 * MeasMashGrav2 * MeasMashGrav2;
	...
	MeasPostPlato =
		-616.868 + 1111.14 * MeasPostSG - 630.272 * MeasPostSG * MeasPostSG + 135.997 * MeasPostSG * MeasPostSG * MeasPostSG;
	...
	TempAdjustmentStrike =
		(4.13643 * Math.pow(10, -16) * Math.pow($("#MeasuredStrikeTemp").val(), 6) -
			4.05998 * Math.pow(10, -13) * Math.pow($("#MeasuredStrikeTemp").val(), 5) +
			1.61536 * Math.pow(10, -10) * Math.pow($("#MeasuredStrikeTemp").val(), 4) -
			3.44854 * Math.pow(10, -8) * Math.pow($("#MeasuredStrikeTemp").val(), 3) +
			0.00000532769 * Math.pow($("#MeasuredStrikeTemp").val(), 2) -
			0.000292675 * $("#MeasuredStrikeTemp").val() +
			1.00493) /
		1.0019647220067072;
	...
	TempAdjustmentBoil =
		(4.13643 * Math.pow(10, -16) * Math.pow($("#MeasuredPreboilTemp").val(), 6) -
			4.05998 * Math.pow(10, -13) * Math.pow($("#MeasuredPreboilTemp").val(), 5) +
			1.61536 * Math.pow(10, -10) * Math.pow($("#MeasuredPreboilTemp").val(), 4) -
			3.44854 * Math.pow(10, -8) * Math.pow($("#MeasuredPreboilTemp").val(), 3) +
			0.00000532769 * Math.pow($("#MeasuredPreboilTemp").val(), 2) -
			0.000292675 * $("#MeasuredPreboilTemp").val() +
			1.00493) /
		1.0019647220067072;
	...
	TempAdjustmentChilled =
		(4.13643 * Math.pow(10, -16) * Math.pow($("#MeasuredChilledVol").val(), 6) -
			4.05998 * Math.pow(10, -13) * Math.pow($("#MeasuredChilledVol").val(), 5) +
			1.61536 * Math.pow(10, -10) * Math.pow($("#MeasuredChilledVol").val(), 4) -
			3.44854 * Math.pow(10, -8) * Math.pow($("#MeasuredChilledVol").val(), 3) +
			0.00000532769 * Math.pow($("#MeasuredChilledVol").val(), 2) -
			0.000292675 * $("#MeasuredChilledVol").val() +
			1.00493) /
		1.0019647220067072;

	MeasuredChilledTempVol = MeasuredChilledVol / TempAdjustmentChilled;
	byId("MeasuredChilledTempVol").value = MeasuredChilledTempVol;
	...
	MashSG =
		...
		100 *
			MAGPot *
			GBill /
			(MAGPot * GBill + SGSuccrose * 8.3304 * MeasuredStrikeVol) /
			(258.6 - 227.1 * (100 * MAGPot * GBill / (MAGPot * GBill + SGSuccrose * 8.3304 * MeasuredStrikeVol) / 258.2));
	...
	if (MeasuredConversion > 0 && MeasuredPreboilGrav > 1.001 && MeasuredPreboilTempVol > 0.1) {
		...
		MeasuredSparge =
			100 * (MeasuredLauter / 100 - EstLauterFirstRunEffArray[GBillArrayIndex]) / EstLauterSecRunEffArray[GBillArrayIndex];
	} else {
		BoilGravPoints = 0;
		MeasuredMash = 0;
		MeasuredLauter = 0;
...
function updateCalc() {
	...
	if (radio !== "metric") {
		...
		if (Math.abs(WaterTot - PreviousWaterTot) > 0.01) {
			if (control1 ="mashthickness"){
				ChangeInVolSparge();
			}
						if (control1 ="VolSparge"){
				ChangeInMashThickness();
			}
		}


		if (Math.abs(MashThickness - PreviousThickness) > 0.01) {
ChangeInMashThickness();
		}

		if (Math.abs(VolSparge - PreviousSparge) > 0.01) {
ChangeInVolSparge();
		}
						if (MashThickness <= 0.5) {
			MashThickness = 0.5;
ChangeInMashThickness();
		}
	...
	//TODO add sparge - thickness relationship to metric later
	...
	if (radio == "metric") {
		...
		VolStrike = VolStrike / 0.264172052358148;
