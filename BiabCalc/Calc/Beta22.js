...
function updateInputs() {
  ...
  MeasuredFirstRunningsGrav = parseFloat(
    $("#MeasuredFirstRunningsGrav").val()
  ).toFixed(3);
...
function LauterEfficiency() {
  ...
  MaxLauterGBill =
    -(2 * (LossBoil + BatchVol + LossTunTrub + LossTrub + LossHop)) /
    (4 * Gabs - 0.825);
  while (LauterGBill < MaxLauterGBill) {
    ...
    LauterVolSparge =
      (BatchVol + LossHop + LossBoil + LossTrub + LossTunTrub) / 2;
    ...
    EstLauterFirstRunningsVolume =
      LauterVolStart - LauterGBill * Gabs - LossTunTrub;
    ...
    EstLauterFirstRunEff =
      EstLauterFirstRunEff * (VolChilled / (VolPost / 1.043841336));
...
function MashAnalysis() {
  ...
  if (SecRun > 0) {
    ...
    EstSpargeEff =
      SpargeCoeff * ((EstLauterEff - NoSpargeLauter) / LauterGained);
  ...
  PlatoPost =
    100 *
    RCSTot /
    (RCSTot + RecW2 + RCWtr1 - BoilRate * 8.3304 * (BoilTime / 60));
  SGPost = 1 + PlatoPost / (258.6 - 0.879551 * PlatoPost);
  TempMashout =
    (TempMash * (GBill + 5 * (GBill * Gabs)) + 5 * TempSparge * VolSparge) /
    (GBill + 5 * (VolSparge + GBill * Gabs));
  ...
  MeasGabs = Gabs;
  MeasMashWT =
    -((VolStart * 8.335 + GBill * MAGMoist) * MeasMashPlato) /
    (-100 + MeasMashPlato);
  ...
  MeasuredSpargeEfficiency = Math.max(
    0,
    100 * ((MeasLautEff - NoSpargeLauter) / LauterGained)
  );
  ...
  MeasSecRunSG = Math.max(
    0,
    1 + MeasSecRunPlato / (258.6 - 0.879551 * MeasSecRunPlato)
  );
  MeasPostSG =
    1 +
    (MeasPrebGrav2 - 1) *
      1000 *
      (MeasPrebVolume / 1.043841336) /
      (VolPost / 1.043841336) /
      1000;
  MeasPostPlato =
    -616.868 +
    1111.14 * MeasPostSG -
    630.272 * MeasPostSG * MeasPostSG +
    135.997 * MeasPostSG * MeasPostSG * MeasPostSG;
  ...
  TempAdjustmentStrike =
    (4.13643 * Math.pow(10, -16) * Math.pow($("#MeasuredStrikeTemp").val(), 6) -
      4.05998 *
        Math.pow(10, -13) *
        Math.pow($("#MeasuredStrikeTemp").val(), 5) +
      1.61536 *
        Math.pow(10, -10) *
        Math.pow($("#MeasuredStrikeTemp").val(), 4) -
  ...
  TempAdjustmentBoil =
    (4.13643 *
      Math.pow(10, -16) *
      Math.pow($("#MeasuredPreboilTemp").val(), 6) -
      4.05998 *
        Math.pow(10, -13) *
        Math.pow($("#MeasuredPreboilTemp").val(), 5) +
      1.61536 *
        Math.pow(10, -10) *
        Math.pow($("#MeasuredPreboilTemp").val(), 4) -
      3.44854 *
        Math.pow(10, -8) *
        Math.pow($("#MeasuredPreboilTemp").val(), 3) +
  ...
  TempAdjustmentChilled =
    (4.13643 * Math.pow(10, -16) * Math.pow($("#MeasuredChilledVol").val(), 6) -
      4.05998 *
        Math.pow(10, -13) *
        Math.pow($("#MeasuredChilledVol").val(), 5) +
      1.61536 *
        Math.pow(10, -10) *
        Math.pow($("#MeasuredChilledVol").val(), 4) -
  ...
  FirstRunningGravPoints =
    (MeasuredFirstRunningsGrav - 1) * 1000 * MashWortVolume;
  if (
    GBill > 0.1 && MeasuredStrikeVol > 0.1 && MeasuredFirstRunningsGrav > 1.001
  ) {
  ...
  if (
    MeasuredConversion > 0 &&
    MeasuredPreboilGrav > 1.001 &&
    MeasuredPreboilTempVol > 0.1
  ) {
    ...
    MeasuredSparge =
      100 *
      (MeasuredLauter / 100 - EstLauterFirstRunEffArray[GBillArrayIndex]) /
      EstLauterSecRunEffArray[GBillArrayIndex];
  } else {
    BoilGravPoints = 0;
    MeasuredMash = 0;
    MeasuredLauter = 0;
  ...
  if (
    MeasuredMash > 0 &&
    MeasuredChilledGrav > 1.001 &&
    MeasuredChilledTempVol > 0.1
  ) {
    ChilledGravPoints =
      (MeasuredChilledGrav - 1) * 1000 * MeasuredChilledTempVol;
  ...
  //feedback scores
  MeasuredLauterFeedback =
    MeasuredLauter / (100 * LauterArray[GBillArrayIndex]);
  MeasuredSpargeFeedback = MeasuredSparge / 10;
  if (MeasuredConversion < 1) {
    MeasuredMashFeedback = 0;
  } else {
    MeasuredMashFeedback =
      MeasuredMash / (LauterArray[GBillArrayIndex] * MeasuredConversion);
  }
  if (MeasuredMash < 1) {
...
function updateCalc() {
 ...
 {Control1 = 2; */
  ...
  VolMinSparge = Math.max(
    0,
    (WaterTot + GBill * 0.08) * MashAdj - (PotSize - 0.1)
  );
  HPost = GalH * VolPost;
...
var ImportData = function ImportData() {
  ...
  if (DataName.length < 60) {
    ...
    byId("DataName").value =
      "Import of " + byId("BeerName").value + " Complete";
  }
  updateCalc();
};
