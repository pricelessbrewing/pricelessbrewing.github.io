  ...
  function updateCalc() {
    ...
    var BatchVol = parseFloat($('#BatchVol').val()),
      GBill = parseFloat($('#GBill').val()),
        LossMashT = parseFloat($('#LossMashT').val()),
      Conv = parseFloat($('#Conv').val()),
      ...
      LossTot = LossGrain + LossHop + LossBoil + LossTrub + LossMashT,
      ...
      VolPre = (WaterTot - LossGrain - LossMashT ) * 1.043841336,
      ...
      SugarGEQ = SugarTot / GBill;

    var MSW1 = MAGDryG * ExConv,
      Plato1 = (100 * MSW1) / (MSW1 + MashWaterWt1),
      SG1 = 1 + (Plato1 / (258.6 - 0.879551 * Plato1)),
      RW1 = (SG1 * ((FirstRun - LossMashT) / 1.022494888)  * 8.3304),
      RS1 = (RW1 * Plato1) / 100,
      RCWtr1 = ( RW1 - RS1 ),
      RetS1 = MSW1 - RS1,
      RetS1GEQ = RetS1 * SugarGEQ,
      RetWat1 = MashWaterWt1 - RCWtr1,
      ...
      RetSF = RetS2,
      RetSFGEQ = RetSF * SugarGEQ,
      TotalPoints = ((VolPre / 1.044) * (SGPre - 1) * 1000),
      PlatoPost = (1.015 * 100 * RCSTot) / (RCSTot + RecW2 + RCWtr1 - (BoilRate * 8.3304 * (BoilTime / 60))),
      SGPost = 1 + (((SGPre - 1) * 1000) * VolPre / VolChilled) / 1000,
    EstBHEff = EstMashEff * ( ( VolChilled - LossTrub ) / VolChilled);
    ...
    $('#RetSFGEQ').text(RetSFGEQ.toFixed(2));
    $('#RetS1GEQ').text(RetS1GEQ.toFixed(2));
    $('#RetS2GEQ').text(RetS2GEQ.toFixed(2));
     $('#LossMashT').text(RetSF.toFixed(2));
      $('#EstBHEff').text(EstBHEff.toFixed(2));
